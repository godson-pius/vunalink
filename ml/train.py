...
evaluation = model.evaluate(test, return_dict=True); output = Path(args.output_dir); output.mkdir(parents=True, exist_ok=True); model.export(output / "saved_model")
...
(output / "evaluation.json").write_text(json.dumps({"dataset": "PlantVillage", "metrics": {k: float(v) for k, v in evaluation.items()}, "note": "Not representative of real-world Rwanda field accuracy."}, indent=2) + "\n")
